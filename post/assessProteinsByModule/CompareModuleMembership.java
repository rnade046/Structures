...
public class CompareModuleMembership {
	...
	public static HashMap<String, Module> loadModules(String membershipFile){
		...
		try {
			...
			List<HashSet<String>> proteinSets = new ArrayList<>();
			for(int i=0; i<modules.length; i++) {
				proteinSets.add(new HashSet<>());
			}
			line = in.readLine();
		...
		try {
			...
			for(Entry<String, Module> m : modules1.entrySet()) {
				if(modules2.containsKey(m.getKey())) {
					...
					HashSet<String> proteins1 = m.getValue().getProteins();
					HashSet<String> proteins2 = modules2.get(m.getKey()).getProteins();
